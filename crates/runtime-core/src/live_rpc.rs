...
#[derive(Clone)]
struct LiveRpcStateOwner {
    runtime_core: RuntimeCoreHandle,
    simulation_service: LiveRpcSimulationService,
}
...
impl LiveRpcStateOwner {
    fn runtime_core(handle: RuntimeCoreHandle) -> Self {
        let queue_capacity = resolve_sim_queue_capacity();
        let worker_total = resolve_sim_worker_count();
        Self {
            runtime_core: handle,
            simulation_service: LiveRpcSimulationService::new(queue_capacity, worker_total),
        }
    }

    fn handle(&self) -> &RuntimeCoreHandle {
        &self.runtime_core
    }

    fn reset_drop_metrics(&self) {
        self.handle().reset_drop_metrics();
    }

    fn observe_drop_reason(&self, reason: LiveRpcDropReason) {
    ...
    fn configure_simulation_queue(&self, queue_capacity: usize, worker_total: usize) {
        self.handle()
            .configure_simulation_queue(queue_capacity, worker_total);
    }

    fn observe_simulation_enqueue(&self) {
        self.handle().observe_simulation_enqueue();
    }

    fn observe_simulation_dequeue(&self) {
        self.handle().observe_simulation_dequeue();
    }

    fn observe_simulation_finish(&self) {
        self.handle().observe_simulation_finish();
    }

    fn observe_simulation_queue_full_drop(&self) {
        self.handle().observe_simulation_queue_full_drop();
    }

    fn observe_simulation_stale_drop(&self) {
        self.handle().observe_simulation_stale_drop();
    }

    fn observe_simulation_cache_hit(&self) {
        self.handle().observe_simulation_cache_hit();
    }

    fn observe_simulation_cache_miss(&self) {
        self.handle().observe_simulation_cache_miss();
    }

    fn observe_simulation_result(
        &self,
        status: RemoteSimulationStatus,
        latency_ms: u64,
        tx_count: usize,
        ...
        self.handle().observe_simulation_result(
    ...
    fn record_simulation_status(&self, snapshot: LiveRpcSimulationStatusSnapshot) {
        self.handle().record_simulation_status(snapshot);
    }

    fn reset_chain_status(&self) {
...
fn spawn_live_rpc_simulation_runtime(
    ...
    for _ in 0..worker_total {
        tokio::spawn(run_simulation_worker(receiver.clone()));
    }

    LiveRpcSimulationRuntime { ingress_tx }
}

impl LiveRpcSimulationService {
    fn new(queue_capacity: usize, worker_total: usize) -> Self {
        Self {
...
async fn run_simulation_worker(receiver: Arc<Mutex<mpsc::Receiver<SimulationTask>>>) {
    loop {
        let task = {
        ...
        let outcome = simulate_remote_request_with_owner(
            &task.state_owner,
            task.state_owner.simulation_http_client(),
            &task.chain,
            &task.request,
        )
        .await;
...
fn build_simulation_status_snapshot(
    task: &SimulationTask,
    outcome: &RemoteSimulationOutcome,
...
fn apply_builder_assembly_state_with_owner(
    state_owner: &LiveRpcStateOwner,
    task: &SimulationTask,
    outcome: &RemoteSimulationOutcome,
) {
...
impl LiveRpcStateOwner {
    fn update_chain_status(
        ...
        let last_pending_unix_ms = if observed_pending || state == "subscribed" {
            ...
            existing
                .as_ref()
                .and_then(|status| status.last_pending_unix_ms)
        };
        let rotation_count = existing
            .as_ref()
...
pub fn start_live_rpc_feed_with_runtime_core(
    ...
    start_live_rpc_feed_with_owner(
        LiveRpcStateOwner::runtime_core(runtime_core),
        config,
        bootstrap_from_pending_pool,
    );
}

fn start_live_rpc_feed_with_owner(
    state_owner: LiveRpcStateOwner,
    config: LiveRpcConfig,
    bootstrap_from_pending_pool: bool,
) {
    state_owner.handle().record_live_rpc_feed_start();
    ...
    let next_seq_id = Arc::new(AtomicU64::new(
        current_seq_hi(state_owner.handle().storage())
            .saturating_add(1)
            .max(1),
    ));
...
fn start_live_rpc_pending_pool_rebuild_with_owner(
    state_owner: LiveRpcStateOwner,
    config: LiveRpcConfig,
) {
    let handle = match tokio::runtime::Handle::try_current() {
    ...
    for chain in config.chains {
        ...
        let scheduler = state_owner.handle().scheduler().clone();
        handle.spawn(async move {
            run_chain_pending_pool_rebuild(state_owner, writer, scheduler, chain, next_seq_id)
                .await;
        });
    }
}
...
async fn run_chain_worker(
    ...
    if bootstrap_from_pending_pool {
        match rebuild_scheduler_from_pending_pool_with_owner(
            &state_owner,
            &writer,
            &scheduler,
            &chain,
                    ...
                    Message::Text(text) => {
                        if let Some(hash_hex) = parse_pending_hash(&text, &mut subscription_id) {
                            ...
                            pending_hashes.push_back(PendingHashObservation {
                                ...
                                observed_at_unix_ms,
                                observed_at_mono_ns: session.state_owner.current_mono_ns(),
                            });
    ...
    for observation in hash_batch {
        let hash = match parse_fixed_hex::<32>(&observation.hash_hex) {
            ...
            None => {
                session
                    .state_owner
                    .observe_drop_reason(LiveRpcDropReason::InvalidPendingHash);
                tracing::warn!(
                    chain_key = %session.chain.chain_key,
...
async fn process_pending_hash_with_fetched_tx_with_owner(
    ...
    if let Some(tx) = fetched_tx {
        ...
        for transition in queue_transitions {
            if !append_queue_transition_event_with_owner(
                state_owner,
                writer,
                chain,
                next_seq_id,
...
async fn rebuild_scheduler_from_pending_pool_with_owner(
    state_owner: &LiveRpcStateOwner,
    writer: &StorageWriteHandle,
    scheduler: &SchedulerHandle,
    chain: &ChainRpcConfig,
    ...
    for endpoint in &chain.endpoints {
        match fetch_pending_block_transactions(client, endpoint.http_url.as_str()).await {
            Ok(pending) => {
                return rebuild_scheduler_from_pending_transactions_with_owner(
                    state_owner,
                    writer,
                    scheduler,
                    chain,
...
async fn rebuild_scheduler_from_pending_transactions_with_owner(
    state_owner: &LiveRpcStateOwner,
    writer: &StorageWriteHandle,
    scheduler: &SchedulerHandle,
    chain: &ChainRpcConfig,
...
fn append_queue_transition_event_with_owner(
    state_owner: &LiveRpcStateOwner,
    writer: &StorageWriteHandle,
    chain: &ChainRpcConfig,
    next_seq_id: &Arc<AtomicU64>,
...
fn append_event_with_owner(
    ...
    try_enqueue_storage_write_with_owner(
        state_owner,
        writer,
        chain,
        StorageWriteOp::AppendPayload {
...
fn try_enqueue_storage_write_with_owner(
    state_owner: &LiveRpcStateOwner,
    writer: &StorageWriteHandle,
    chain: &ChainRpcConfig,
    op: StorageWriteOp,
    ...
    match writer.try_enqueue(op) {
        ...
        Err(error) => {
            ...
            state_owner.observe_drop_reason(reason);
            tracing::warn!(
                chain_key = %chain.chain_key,
                source_id = %chain.source_id,
...
async fn simulate_remote_request_with_owner(
    state_owner: &LiveRpcStateOwner,
    client: &reqwest::Client,
    chain: &ChainRpcConfig,
    request: &RemoteSimulationRequest,
    ...
    let simulated = tokio::time::timeout(timeout, async {
        ...
        let account_seeds = fetch_cached_account_seeds(
            state_owner,
            client,
            http_url,
            chain_context.block_number,
            &request.txs,
        )
        .await?;
    ...
    state_owner.observe_simulation_result(status, latency_ms, tx_count, fail_category.as_deref());
...
async fn fetch_cached_account_seeds(
    ...
    for sender in unique_senders {
        if let Some(seed) = state_owner.handle().cached_account_seed(
            http_url,
            sender,
            block_number,
            now_unix_ms,
            ...
            state_owner.observe_simulation_cache_hit();
            seeds.insert(sender, seed);
            continue;
        }

        state_owner.observe_simulation_cache_miss();
        let seed = fetch_account_seed_from_rpc(client, http_url, sender).await?;
        state_owner
            .handle()
            .cache_account_seed(http_url, sender, block_number, now_unix_ms, seed);
        seeds.insert(sender, seed);
    }
...
mod tests {
    ...
    fn test_runtime_core_owner(
        writer: &StorageWriteHandle,
        scheduler: &SchedulerHandle,
    ) -> (RuntimeCoreHandle, LiveRpcStateOwner) {
        ...
        .expect("runtime core");
        let owner = LiveRpcStateOwner::runtime_core(handle.clone());
        (handle, owner)
    }
    ...
    async fn remote_simulation_request_uses_cache_until_block_number_changes() {
        ...
        let client = state_owner.simulation_http_client().clone();

        let first = simulate_remote_request_with_owner(&state_owner, &client, &chain, &request)
            .await
            .expect("first simulation");
        let second = simulate_remote_request_with_owner(&state_owner, &client, &chain, &request)
            .await
        ...
        let third = simulate_remote_request_with_owner(&state_owner, &client, &chain, &request)
            .await
    ...
    async fn remote_simulation_request_classifies_rpc_state_errors() {
        ...
        let outcome = simulate_remote_request_with_owner(
            &state_owner,
            state_owner.simulation_http_client(),
            &chain,
            &request,
        )
        .await
    ...
    async fn process_pending_hash_with_runtime_core_owner_updates_runtime_core_views() {
        ...
        let chain = test_chain_with_http_url(format!("http://{rpc_addr}"));
        let tx = sample_swap_live_tx(0xc7, 0x47, 7, 120);
        let next_seq_id = Arc::new(AtomicU64::new(1));
        ...
        let _ops = drain_storage_ops_until_quiet(&mut storage_rx, 250).await;
        let metrics = runtime_core.simulation_metrics();
        assert_eq!(metrics.completed_total, 1);
        assert_eq!(metrics.ok_total, 1);
        assert_eq!(runtime_core.builder_snapshot().candidates.len(), 1);
        ...
        assert_eq!(
            runtime_core
                .simulation_status("latest")
                .expect("latest simulation status")
                .status,
            "ok"
        );
        assert!(runtime_core.searcher_metrics().executable_candidates_total >= 1);
        assert_eq!(rpc_state.balance_requests.load(Ordering::SeqCst), 1);
        assert_eq!(rpc_state.nonce_requests.load(Ordering::SeqCst), 1);
    ...
    #[tokio::test]
    async fn live_rpc_feed_start_count_is_runtime_core_scoped() {
        let (storage_tx, _storage_rx) = tokio::sync::mpsc::channel(8);
        let writer = StorageWriteHandle::from_sender(storage_tx);
        let (scheduler, _runtime) =
            scheduler::scheduler_channel(scheduler::SchedulerConfig::default());
        ...
        .expect("first runtime core");
        let second = RuntimeCore::start(RuntimeCoreStartArgs {
            deps: RuntimeCoreDeps {
                storage: Arc::new(RwLock::new(InMemoryStorage::default())),
                writer,
                scheduler,
        ...
        assert_eq!(first.live_rpc_feed_start_count(), 1);
        assert_eq!(second.live_rpc_feed_start_count(), 0);
    }
    ...
    async fn rebuild_scheduler_from_pending_transactions_rehydrates_scheduler_and_emits_events() {
        ...
        let rebuilt = rebuild_scheduler_from_pending_transactions_with_owner(
            &state_owner,
            &writer,
            &scheduler,
            &chain,
